...
#         # Get all concept IDs from includes using batch processing
#         all_concept_ids = set()
        
#         # Process all includes in batches
...
#         logger.error(f"ValueSet expand error: {str(e)}")
#         return Response({
#             "resourceType": "OperationOutcome",
#             "issue": [{
#                 "severity": "error",
...
# def find_descendants_batch(concept_id, max_depth=10):
#     """Find all descendants using batch queries to avoid recursion"""
#     all_descendants = set()
...
#         # Get all direct children in one query
#         try:
#             resp = es.search(
#                 index="relationships",
#                 body={
...
#     """Filter concepts by text using batch query"""
#     if not filter_text or not concept_ids:
#         return concept_ids
    
#     try:
#         # Build query for text filtering
#         query = {
#             "query": {
#                 "bool": {
#                     "must": [
...
#                             "bool": {
#                                 "should": [
#                                     {"wildcard": {"term": f"*{filter_text.lower()}*"}},
...
#     """Get concept details using batch queries"""
#     if not concept_ids:
#         return []
    
#     try:
#         # Get descriptions for all concepts in one query
#         descriptions_query = {
#             "query": {
#                 "bool": {
#                     "must": [
...
#             "size": len(concept_ids) * 10  # Assume max 10 descriptions per concept
#         }
...
def expand_view(request):
    ...
    try:
        ...
        logger.info(f"Total concept IDs before filtering: {len(all_concept_ids)}")
...
def filter_concepts_by_text_batch(concept_ids, filter_text, display_language):
    ...
    try:
        ...
        language_codes = [display_language, "en"]
        if display_language == "en-gb":
            language_codes = ["en-gb", "en"]
        elif display_language == "en-us":
        ...
        for lang_code in language_codes:
                ...
                "query": {
                    "bool": {
                        "must": [
                            {"terms": {"concept_id": concept_ids}},
                            {"term": {"active": True}},
                            {"term": {"language_code": lang_code}},
                            {
                                "bool": {
                                    "should": [
                                        {"wildcard": {"term": f"*{filter_text.lower()}*"}},
                                        {"match": {"term": {"query": filter_text, "fuzziness": "AUTO"}}},
                                        {"prefix": {"term": filter_text.lower()}}
                                    ]
                                }
                            }
                        ]
            ...
            logger.info(f"Found {len(matching_concept_ids)} concepts with text '{filter_text}' in language '{lang_code}'")
